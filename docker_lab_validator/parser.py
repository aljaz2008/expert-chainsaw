...
FEATURE_PATTERNS: list[tuple[re.Pattern[str], str, str]] = [
    (re.compile(r"\biptables\b|DOCKER-USER", re.I), "iptables", "iptables/DOCKER-USER is Linux-specific and may require root"),
    (re.compile(r"\bnft\b|\bnftables\b", re.I), "nftables", "nftables is Linux-specific and may require root"),
    (re.compile(r"\bip\s+netns\b|/var/run/netns", re.I), "ip_netns", "Linux network namespaces require iproute2 and elevated privileges"),
    (re.compile(r"\bconntrack\b", re.I), "conntrack", "conntrack requires Linux conntrack tooling and privileges"),
    ...
    (re.compile(r"\btcpdump\b", re.I), "tcpdump", "packet capture requires tcpdump and capture privileges"),
    (re.compile(r"\bebpf\b|\bbpftrace\b|\bbpftool\b", re.I), "ebpf", "eBPF observability requires a Linux kernel with BPF support"),
    (re.compile(r"\btetragon\b", re.I), "tetragon", "Tetragon must be installed in a compatible Linux/Kubernetes environment"),
    ...
    (re.compile(r"\bAAAA\b|--ipv6|enable_ipv6|fixed-cidr-v6", re.I), "ipv6", "IPv6 Docker networking must be enabled"),
]
